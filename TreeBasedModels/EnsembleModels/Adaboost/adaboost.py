...
def amount_of_say(tot_err):
    aos = 0.5 * math.log((1-tot_err) / tot_err)
    return aos

def new_sample_weight(sample_weight, amount_of_say):
    new_sample_weight = sample_weight * math.exp(amount_of_say)
...
dff = df.drop(["weight", "Play?"], axis=1)

# print(make_stump(df, "Numberplayers", 4, "Play?"))
# print(make_categorical_stump(df, "Temperature", "Hot", "Play?"))
for col in dff.columns:
    if df[col].dtype in ["int64", "float64"]:
        pass
        # print(compute_gini(df[[col,"Play?"]],"Play?"))
    else:
        print(compute_gini(df[[col,"Play?"]],"Play?"))

        # print(compute_gini(df[[subset,"Play?"]],"Play?"))
